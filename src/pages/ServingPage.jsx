...
export default function ServingPage() {
  ...
  useEffect(() => {
    if (!breakEvenAge || !showResults) return;
    ...
    let delay = 1000;
    try { if (!sessionStorage.getItem("milcalc_debriefed_shown")) delay += 5000; } catch {}
    const t = setTimeout(() => {
      try { if (sessionStorage.getItem("milcalc_survey_shown")) return; } catch {}
  ...
  }, [breakEvenAge, showResults]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function handleSurveySubmit() {
    track("Feedback Submitted", {
      text: surveyText, source: "micro_survey", trigger: "stay_vs_go",
      path: "serving", timestamp: new Date().toISOString(),
    });
    setSurveyDone(true);
    try { sessionStorage.setItem("milcalc_survey_shown", "1"); } catch {}
    setTimeout(() => setShowSurvey(false), 2000);
  ...
  const generatePDF = () => {
    try {
      ...
      const hdrBar = () => {
        ...
        doc.text(svgName ? `Transition Plan for ${svgName} · ${PUBLIC_DOMAIN}` : `Transition Plan · ${PUBLIC_DOMAIN}`,M,60);
  ...
  const doShare = async () => {
    ...
    const shareData = { title: "My Stay vs Go — MilCalc", text: `I ran the numbers on staying vs getting out. Calculate yours free at ${PUBLIC_DOMAIN}`, url: PUBLIC_URL };
    const doDownload = () => { const url = URL.createObjectURL(shareBlobRef.current); const a = document.createElement("a"); a.href = url; a.download = "milcalc-stayvsgo.png"; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); };
      ...
      <div className="sp-wrap">
        <div className="sp-inner">
          ...
          <div className="ds-card">
            ...
            {/* Health insurance */}
            <div className="sp-row">
              <span className="sp-row-lbl">Health Insurance</span>
              <div className="sp-row-val">
                <ToggleGroup
            ...
            <div className="sp-stack">
              <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:8 }}>
                <span className="sp-lbl">TRICARE Plan</span>
                <button
                  type="button"
          ...
          <SectionHeader>GI Bill</SectionHeader>
          <div className="ds-card">
            <div className="sp-row">
                      ...
                      {(surveyChoice === "not_really" || surveyChoice === "suggest") && (
                        <div className="survey-expand">
                          <textarea className="survey-textarea"
                            ...
                            value={surveyText} onChange={e => setSurveyText(e.target.value)}
                            rows={3} autoFocus />
