...
import {
  PUBLIC_URL, PUBLIC_DOMAIN, SUPPORT_EMAIL,
  PARENT_BRAND_URL, PARENT_BRAND_DOMAIN,
} from "./config.js";
...
html{-webkit-text-size-adjust:100%;-webkit-tap-highlight-color:transparent;padding-top:0;background:#0f0f14}
body{background:var(--bg);color:var(--ink);font-family:Barlow,sans-serif;
  ...
  -webkit-font-smoothing:antialiased;padding-top:0;background:#0f0f14}
...
.sb{position:fixed;top:0;left:0;right:0;
  height:calc(var(--sh) + max(env(safe-area-inset-top),16px));padding-top:max(env(safe-area-inset-top),16px);
  background:var(--sub);
...
/* ── MAIN CONTENT ── */
.main{margin-top:calc(var(--sh) + max(env(safe-area-inset-top),16px));
  padding:20px 16px calc(var(--tabh) + env(safe-area-inset-bottom,0px) + 20px);
...
.db-badge{position:fixed;top:env(safe-area-inset-top,0px);left:0;right:0;z-index:300;
...
.has-badge .sb{top:calc(28px + env(safe-area-inset-top,0px));height:var(--sh);padding-top:0}
.has-badge .main{margin-top:calc(var(--sh) + env(safe-area-inset-top,0px) + 28px)}
...
@media(min-width:768px){
  .main{padding:28px 32px calc(var(--tabh) + env(safe-area-inset-bottom,0px) + 28px);max-width:720px}
...
@media(min-width:1024px){
  .main{max-width:1060px;padding:32px 40px calc(var(--tabh) + env(safe-area-inset-bottom,0px) + 32px)}
...
.info-sheet{background:var(--card);border-radius:16px 16px 0 0;width:100%;
  max-width:480px;padding:24px 20px calc(20px + env(safe-area-inset-bottom,0px));
...
function DebriefedGapCard({ gap }) {
  ...
  return (
    <div style={{ background:"#1a2744", backgroundImage:DEBRIEFED_TEXTURE, borderRadius:16, padding:24, border:"1px solid rgba(255,255,255,0.06)", boxShadow:"0 2px 12px rgba(0,0,0,0.3)", fontFamily:"'Barlow',sans-serif", margin:"16px 0" }}>
      ...
      <a href={PARENT_BRAND_URL} target="_blank" rel="noopener noreferrer"
...
function DashboardTab({state,set,isConfigured,go,onPdfExported,modalActive}){
  ...
  const buildInfographicCanvas=()=>{
    ...
    const fT=PUBLIC_DOMAIN,fX=W-PAD;ctx.fillText(fT,fX,y);
  ...
  // ── Pre-generate blob when results are ready — so share tap is synchronous ──
  useEffect(()=>{
    console.log('[RetiredTab] Building canvas blob, showShareBtn:', showShareBtn);
    if(!showShareBtn) return;
    const canvas=buildInfographicCanvas();
    canvas.toBlob(blob=>{
      console.log('[RetiredTab] Canvas blob generated, size:', blob?.size);
      if(blob) shareBlobRef.current=blob;
    },"image/png");
  },[atP,vaM,mhaMo,otherMo,sbpC,healthPrem,civHiCost,vgliPrem,otherLifePrem,desiredIncome]);

  // ── Share handler — navigator.share called synchronously from gesture ──
  const handleShareMyResults=()=>{
    console.log('[RetiredTab] Share tapped');
    console.log('[RetiredTab] navigator.share available:', !!navigator.share);
    console.log('[RetiredTab] blob available:', !!shareBlobRef.current);
    console.log('[RetiredTab] blob size:', shareBlobRef.current?.size);
    const blob=shareBlobRef.current;
    const shareText=`I just calculated my military retirement pay. Calculate yours free at ${PUBLIC_DOMAIN}`;
    console.log('[RetiredTab] shareText:', shareText);
    track("Share Tapped",{});
    if(navigator.share){
      const canShareFile=blob&&navigator.canShare?.({files:[new File([blob],"milcalc-results.png",{type:"image/png"})]});
      console.log('[RetiredTab] canShare files:', canShareFile);
      if(canShareFile){
        console.log('[RetiredTab] Trying file share');
        const file=new File([blob],"milcalc-results.png",{type:"image/png"});
        navigator.share({files:[file],title:"My MilCalc Results",text:shareText})
          .then(()=>console.log('[RetiredTab] File share success'))
          .catch(err=>{
            console.log('[RetiredTab] File share error:', err.name, err.message);
            if(err.name==="AbortError") return;
            navigator.share({title:"MilCalc",text:shareText,url:PUBLIC_URL}).catch(()=>{});
          });
      }else{
        console.log('[RetiredTab] Trying URL share');
        navigator.share({title:"MilCalc",text:shareText,url:PUBLIC_URL})
          .then(()=>console.log('[RetiredTab] URL share success'))
          .catch(err=>{
            console.log('[RetiredTab] URL share error:', err.name, err.message);
            if(err.name==="AbortError") return;
          });
      }
    }else{
      console.log('[RetiredTab] No navigator.share — using fallback');
      if(blob){
        const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="milcalc-results.png";a.click();
      }else{
        navigator.clipboard?.writeText(PUBLIC_URL);
      }
    }
    track("Infographic Shared",{method:navigator.share?"native":"download"});
  };
  ...
  return(
    <div className="fu">
      ...
      <div style={{textAlign:"center",marginTop:16,marginBottom:8}}>
        <span style={{fontSize:11,color:"var(--mut)"}}>MilCalc v{APP_VERSION} · By the maker of Debriefed · <a href={PARENT_BRAND_URL} target="_blank" rel="noopener noreferrer" style={{color:"var(--mut)",textDecoration:"none"}}>{PARENT_BRAND_DOMAIN}</a></span>
      </div>
      ...
      {showDebriefedPromo&&!modalActive&&createPortal(
        <div style={{position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",
          ...
          <div style={{position:"relative",background:"var(--card)",borderRadius:16,padding:"28px 24px",maxWidth:400,width:"90%",
            ...
            <div style={{fontSize:14,color:"var(--mut)",lineHeight:1.55,marginBottom:20}}>Translate your military experience into a civilian resume at {PARENT_BRAND_DOMAIN}</div>
  ...
  function generatePDF(){
    ...
    try{
      ...
      const drawHeader=()=>{
        ...
        doc.text(state.userName?`Transition Plan for ${state.userName} · ${PUBLIC_DOMAIN}`:`Transition Plan · ${PUBLIC_DOMAIN}`,M+70,27);
      ...
      const drawFooter=()=>{
        ...
        doc.text(`Generated by MilCalc (${PUBLIC_DOMAIN}) \u00B7 For informational purposes only \u2014 not financial or legal advice.`,M,fy+8);
        doc.text(`Part of the Debriefed family \u00B7 ${PARENT_BRAND_DOMAIN}`,M,fy+18);
      };
      ...
      doc.text(`Update your numbers anytime at ${PUBLIC_DOMAIN}`,W/2,y+18,{align:"center"});
      ...
      doc.setTextColor(...GOLD_P);doc.text(PARENT_BRAND_DOMAIN,M,pY+32);}
...
// ── TAB 4: PROFILE ───────────────────────────────────────────────────
// ProfileTab (the v1.1 7-way retiree UI) was moved to
// src/pages/TransitioningPage.jsx — the actual rendered retirement page.
// App() (this file's default export) is not mounted by main.jsx (only
// SharePage is imported), so the consolidated profile UI lived here as
// dead code. Removed to stop the dead-code/real-code divergence.
    ...
    <div className="modal-screen">
      ...
      <div className="modal-body">
        ...
        <div className="sup-contact">
          ...
          <div className="sup-contact-email">{SUPPORT_EMAIL}</div>
          <div className="sup-contact-note">Typically respond within 1&ndash;2 business days</div>
          <a className="sup-contact-btn" href={`mailto:${SUPPORT_EMAIL}?subject=MilCalc%20Support`}>Send Email</a>
        </div>
...
const PRIVACY_SECTIONS=[
  ...
  {num:"08",title:"Contact",body:`Questions? Email: ${SUPPORT_EMAIL}`},
];
...
export function SharePage(){
  ...
  const url=hasName?`${PUBLIC_URL}?utm_source=${slug}&utm_medium=share&utm_campaign=referral`:PUBLIC_URL;
  const orgSlug=orgName.trim().toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||"partner";
  const orgUrl=`${PUBLIC_URL}?utm_source=${orgSlug}&utm_medium=website&utm_campaign=member-tools`;
...
export function StayVsGoTab({state,set}){
  ...
  const buildCanvas=()=>{
    ...
    ctx.fillStyle=C.goldL;ctx.font="500 12px system-ui,sans-serif";ctx.textAlign="right";const fT=PUBLIC_DOMAIN;ctx.fillText(fT,W-PAD,y);
  ...
  // ── Pre-generate blob when results change — so share tap is synchronous ──
  useEffect(()=>{
    console.log('[StayVsGoTab] Building canvas blob');
    const canvas=buildCanvas();
    canvas.toBlob(blob=>{
      console.log('[StayVsGoTab] Canvas blob generated, size:', blob?.size);
      if(blob) shareBlobRef.current=blob;
    },"image/png");
  },[moA_at65,moB_at65,breakEvenAge,safeSep,safeTgt,chartEnd]);

  // ── Share handler — navigator.share called synchronously from gesture ──
  const handleShare=()=>{
    console.log('[StayVsGoTab] Share tapped');
    console.log('[StayVsGoTab] navigator.share available:', !!navigator.share);
    console.log('[StayVsGoTab] blob available:', !!shareBlobRef.current);
    console.log('[StayVsGoTab] blob size:', shareBlobRef.current?.size);
    const blob=shareBlobRef.current;
    const shareText=`I just compared staying vs going in the military. Calculate yours free at ${PUBLIC_DOMAIN}`;
    console.log('[StayVsGoTab] shareText:', shareText);
    track("SVG Share Tapped",{});
    if(navigator.share){
      const canShareFile=blob&&navigator.canShare?.({files:[new File([blob],"milcalc-stayvsgo.png",{type:"image/png"})]});
      console.log('[StayVsGoTab] canShare files:', canShareFile);
      if(canShareFile){
        console.log('[StayVsGoTab] Trying file share');
        const file=new File([blob],"milcalc-stayvsgo.png",{type:"image/png"});
        navigator.share({files:[file],title:"My Stay vs Go — MilCalc",text:shareText})
          .then(()=>console.log('[StayVsGoTab] File share success'))
          .catch(err=>{
            console.log('[StayVsGoTab] File share error:', err.name, err.message);
            if(err.name==="AbortError") return;
            navigator.share({title:"MilCalc",text:shareText,url:PUBLIC_URL}).catch(()=>{});
          });
      }else{
        console.log('[StayVsGoTab] Trying URL share');
        navigator.share({title:"MilCalc",text:shareText,url:PUBLIC_URL})
          .then(()=>console.log('[StayVsGoTab] URL share success'))
          .catch(err=>{
            console.log('[StayVsGoTab] URL share error:', err.name, err.message);
            if(err.name==="AbortError") return;
          });
      }
    }else{
      console.log('[StayVsGoTab] No navigator.share — using fallback');
      if(blob){
        const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="milcalc-stayvsgo.png";a.click();
      }else{
        navigator.clipboard?.writeText(PUBLIC_URL);
      }
    }
    track("SVG Infographic Shared",{method:navigator.share?"native":"download"});
  };
        ...
        {vaRating >= 30 && (
          <div className="field" style={{marginTop:12,marginBottom:0}}>
            ...
            <select value={svgVaDep} onChange={e=>set("svg_vaDep",e.target.value)}>
              ...
              <option value="sp_2c">+ Spouse + 2 Kids</option>
              <option value="sp_3c">+ Spouse + 3+ Kids</option>
              <option value="s_1c">+ 1 Child</option>
              <option value="s_2c">+ 2 Children</option>
              <option value="s_3c">+ 3+ Children</option>
            </select>
...
function OnboardingScreen({onSelect}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0a1628",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px",paddingTop:"max(env(safe-area-inset-top,0px),24px)",paddingBottom:"max(env(safe-area-inset-bottom,0px),24px)"}}>
    ...
    <>
      ...
      <div className="db-badge">
        ...
        <div className="db-badge-center">
          ...
          <div className="db-badge-txt">Part of <a href={PARENT_BRAND_URL} target="_blank" rel="noopener noreferrer">Debriefed</a></div>
        </div>
      </div>
      ...
      {/* ── MAIN CONTENT ── */}
      <main className="main">
        {/* myinfo tab UI now lives in src/pages/TransitioningPage.jsx (ProfileTab removed) */}
        ...
        <div className="ep-overlay" onClick={dismissPopup}>
          <div className="ep-modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="ep-options">
              <a className="ep-opt" href={`mailto:${SUPPORT_EMAIL}?subject=MilCalc%20Review&body=I%20wanted%20to%20share%20feedback%20about%20MilCalc%3A`}
